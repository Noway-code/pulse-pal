...
from yarppg.yarppg.rppg.roi.region_of_interest import (
    RegionOfInterest,
    get_default_bgmask,
)
...
class HaarCascadeDetector(ROIDetector):
    ...
    def _get_classifier(cls, casc_file: str):
        if casc_file is not None and Path(casc_file).is_file():
            ...
            warnings.warn(
                "cascade file '{}' not found, using default instead"
                "".format(casc_file)
            )
...
class FaceMeshDetector(ROIDetector):
    ...
    def draw_facemesh(
        self, img, multi_face_landmarks, tesselate=False, contour=False, irises=False
    ):
